...
class WC_Pre_Orders_Checkout {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_available_payment_gateways', [ $this, 'maybe_remove_unsupported_gateways' ], 10 );
		...
		add_filter( 'woocommerce_payment_complete_order_status', [
			$this,
			'update_payment_complete_order_status',
		], 10, 2 );
		...
		add_action( 'woocommerce_order_status_on-hold_to_processing', [
			$this,
			'update_manual_payment_complete_order_status',
		] );
		...
		add_action( 'woocommerce_order_status_failed_to_processing', [
			$this,
			'update_manual_payment_complete_order_status',
		] );
		add_action( 'woocommerce_order_status_failed_to_completed', [
			$this,
			'update_manual_payment_complete_order_status',
		] );

		// Filter the thank you page to add release date message.
		add_filter( 'woocommerce_thankyou_order_received_text', [ $this, 'add_release_date_message' ], 10, 2 );
	}
	...
	public function add_release_date_message( $message, $order ) {
		...
		return $message . ' ' . sprintf( __( 'You will be prompted for payment for your order when your pre-order is released%s Your order details are shown below for your reference.', 'wc-pre-orders' ), $availability_date_text );
	}

	/**
	 * Conditionally remove any gateways that don't support pre-orders on the checkout page when the pre-order is
	 * charged upon release. This is done because payment info is not required in this case so displaying
	 * gateways/payment fields is not needed.
	 *
	 * @since 1.0
	 *
