...
class WC_Pre_Orders_Order {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_status_changed', [ $this, 'auto_update_pre_order_status' ], 10, 3 );

		// automatically cancel a pre-order when it's parent order is trashed
		add_action( 'wp_trash_post', [ $this, 'maybe_cancel_trashed_pre_order' ] );

		// get formatted order total when viewing order on my account page
		add_filter( 'woocommerce_get_formatted_order_total', [ $this, 'get_formatted_order_total' ], 10, 2 );

		// adds a 'Release Date' line to pre-order product order items on the thank-you page, emails, my account, etc
		add_filter( 'woocommerce_order_get_items', [ $this, 'add_product_release_date_item_meta' ], 10, 2 );
	}
	 ...
	 * has a payment token. Should be used by supported gateways when processing a pre-order charged upon release,
	 * instead of calling
	...
	public function register_order_status() {
		...
		register_post_status( 'wc-pre-ordered', [
	 ...
	 * 2) Change the pre-order status to 'active' when the order status changes to 'on-hold' -- this ensures the
	 * pre-orders using a gateway that does not call WC_Order::payment_complete() like BACS or Cheque will still show
	 * on the 'Manage Pre-Orders' page
	 * 3) Change the pre-order status to 'cancelled' when the order status changes to 'cancelled' and the order
	 * contains a pre-order
	 *
	 * @since 1.0
	 *
	...
	public static function update_pre_order_status( $order, $new_status, $message = '' ) {
		...
		if ( ! empty( $message ) ) {
			$message .= ! in_array( substr( $message, - 1 ), [ '!', '?', '.', ';', ':' ] ) ? '.' : '';
			$message .= ' ';
		}
			...
			foreach ( $items as $item_id => $item ) {
				...
				if ( 'line_item' === $current_item['type'] && ! empty( $current_item['product_id'] ) ) {
					...
					if ( ! empty( $pre_order_meta ) ) {
						$current_item['item_meta'] += [ $name => [ $pre_order_meta ] ];
					}
				}
