...
class WC_Pre_Orders_Manager {
	...
	public function __construct() {
		...
		if ( 'yes' !== $disable ) {
			...
			add_action( 'wc_pre_orders_completion_check', [ $this, 'check_for_pre_orders_to_complete' ], 10 );

			// hook into same cron event as above and set products that didn't get any orders to a normal product
			add_action( 'wc_pre_orders_completion_check', [ $this, 'check_for_pre_order_products_to_reset' ], 11 );
		}

		// prevent pre-orders with a 'pending' order status from being auto-cancelled
		add_filter( 'woocommerce_cancel_unpaid_order', [ $this, 'maybe_prevent_pending_order_cancel' ], 10, 2 );

		// prevent order stock reduction when a pre-order charged upon release is completed
		add_filter( 'woocommerce_payment_complete_reduce_order_stock', [
			$this,
			'maybe_prevent_payment_complete_order_stock_reduction',
		], 10, 2 );
	}
	...
	public static function get_users_change_status_link( $new_status, $order ) {
		...
		$action_link = add_query_arg( [ 'order_id' => $order_id, 'status' => $new_status ] );
	...
	/**
	 * Sends a notification email (using the built-in 'Customer Note' email template) to all customers who have
	 * pre-ordered the given product
	 *
	 * @since 1.0
	 *
	...
	/**
	 * Change the release date for  pre-orders by updating the availability date for the pre-ordered product to a new
	 * date in the future
	 *
	 * @since 1.0
	 *
	...
	public static function change_release_date_for_all_pre_orders( $product, $new_availability_date, $message = '' ) {
		...
		foreach ( $orders as $order ) {
			...
			do_action( 'wc_pre_orders_pre_order_date_changed', [
				'order'             => $order,
	...
	/**
	 * Completes the pre-order by updating the pre-order status to 'completed' and following this process for handling
	 * payment :
	 *
	 * - for a pre-order charged upon released AND containing a payment token, an action is fired for the supported
	 * gateway to hook into an charge the total payment amount. Note that the supported gateway will then call
	 * WC_Order::payment_complete() upon successful charge
	 *
	 * - for a pre-order charged upon released with no payment token, the order status is changed to 'pending' and an
	 * email is sent containing a link for the customer to come back to and pay for their order
	 *
	 * - for a pre-order charged upfront, the order status is changed to 'completed' or 'processing' based on the same
	 * rules from WC_Order::payment_complete() -- this is because payment_complete() has already occurred for these
	 * order
	 *
	 * @since 1.0
	 *
	...
	public static function can_pre_order_be_changed_to( $new_status, $order ) {
		...
		switch ( $new_status ) {
			case 'cancelled':
				if ( ! in_array( $status, [ 'cancelled', 'completed', 'trash' ] ) ) {
					$can_be_changed = true;
				}
				break;
			case 'completed':
	...
	public static function is_zero_cost_order( $order = null ) {
		...
		if ( is_a( $order, 'WC_Order' ) ) {
			return 0 >= absint( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->total : $order->get_total() );
		}
	...
	/**
	 * Helper method to disable pre-orders for a product, called after the availability date for a pre-order has been
	 * reached and pre-orders are completed
	 *
	 * @since 1.0
	 *
	 ...
	 * Because order stock for pre-orders charged upon release is reduced during initial checkout, this prevents stock
	 * from being reduced twice.
	 *
	 * @since 1.0
	 *
